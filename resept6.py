...
while True:
    try:
        ...
        # Detecta automáticamente cualquier sensor que envíe mensaje con "sensorX:"
        if mensaje.startswith("sensor"):
            sensor, valores = mensaje.split(":")
            temp, hum = valores.split(",")
            print(f"[{sensor}]  Temp: {temp}°C | Hum: {hum}%   → desde {addr[0]}")
        else:
